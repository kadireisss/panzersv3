...
module.exports = async function handler(req, res) {
  ...
  if (action === 'avatar') { const { userimage }=req.body; if (!userimage) return res.status(400).json({ sonuc:'hata' }); await supabase.from('kullanicilar').update({ userimage }).eq('kullaniciadi',user.kul_id); return res.json({ sonuc:'tamam' }); }
